...
void main() {
  ...
  test('selects readonly renderEditable matches native behavior for android',
    ...
    editable.selectWordsInRange(
        from: const Offset(2, 10), cause: SelectionChangedCause.longPress);
    pumpFrame();
    expect(currentSelection.isCollapsed, false);
  ...
  test('moveSelectionUp/DownByLine stays on the current line', () async {
    ...
    editable.moveSelectionDownByLine(SelectionChangedCause.keyboard);
    expect(currentSelection.isCollapsed, true);
    expect(currentSelection.baseOffset, 13);

    // Try moveSelectionDownByLine again and nothing happens because we're
    ...
    editable.moveSelectionUpByLine(SelectionChangedCause.keyboard);
    expect(currentSelection.isCollapsed, true);
    expect(currentSelection.baseOffset, 0);

    // Trying moveSelectionUpByLine does nothing at the leftmost of the field.
    editable.moveSelectionUpByLine(SelectionChangedCause.keyboard);
    expect(currentSelection.isCollapsed, true);
    expect(currentSelection.baseOffset, 0);
    ...
    // Neither moveSelectionUpByLine nor moveSelectionDownByLine do anything
  ...
  // test(
  //     'MongolRenderEditable registers and unregisters raw keyboard listener correctly',
  //     () async {
  //   final TextSelectionDelegate delegate = FakeEditableTextState()
  //     ..textEditingValue = const TextEditingValue(
  //       text: 'how are you',
  ...
  test('arrow keys with selection text and shift', () async {
    ...
    editable.extendSelectionDown(SelectionChangedCause.keyboard);
    expect(currentSelection.isCollapsed, false);
    expect(currentSelection.baseOffset, 2);
    ...
    editable.extendSelectionUp(SelectionChangedCause.keyboard);
    expect(currentSelection.isCollapsed, false);
    expect(currentSelection.baseOffset, 2);
    expect(currentSelection.extentOffset, 3);
  ...
  group('delete', () {
    ...
    test('when as simple text, it should delete the character to the left',
        () async {
      final TextSelectionDelegate delegate = FakeEditableTextState()
        ..textEditingValue = const TextEditingValue(
          text: 'test',
    ...
    test('when has grapheme clusters, it should delete the grapheme cluster',
        () async {
      final TextSelectionDelegate delegate = FakeEditableTextState()
        ..textEditingValue = const TextEditingValue(
          text: '0123👨‍👩‍👦2345',
    ...
    test('when using rtl', () async {
      const text = 'برنامج أهلا بالعالم';
  ...
  group('deleteByWord', () {
    ...
    test('when cursor is after a word, it should delete the whole word',
        () async {
      const String text = 'test with multiple blocks';
      const int offset = 9;
      final TextSelectionDelegate delegate = FakeEditableTextState()
  ...
  group('deleteByLine', () {
    ...
    test('when previous character is a breakline, it should preserve it',
        () async {
      const String text = 'test with\nmultiple blocks';
      const int offset = 10;
      final TextSelectionDelegate delegate = FakeEditableTextState()
  ...
  group('deleteForwardByWord', () {
    ...
    test('when cursor is before a word, it should delete the whole word',
        () async {
      const String text = 'test with multiple blocks';
      const int offset = 10;
      final TextSelectionDelegate delegate = FakeEditableTextState()
  ...
  //       var result = rendering.BoxHitTestResult();
  //       editable.hitTest(result, position: Offset.zero);
  //       // We expect two hit test entries in the path because the MongolRenderEditable
  //       // will add itself as well.
  //       expect(result.path, hasLength(2));
  //       var target = result.path.first.target;
  //       expect(target, isA<TextSpan>());
  //       expect((target as TextSpan).text, 'A');
  //       // Only testing the MongolRenderEditable entry here once, not anymore below.
  //       expect(result.path.last.target, isA<MongolRenderEditable>());

  //       result = rendering.BoxHitTestResult();
  //       editable.hitTest(result, position: const Offset(0.0, .0));
  //       expect(result.path, hasLength(2));
  ...
  group('delete API implementations', () {
    ...
    late MongolRenderEditable editable;
    ...
    setUp(() {
      editable = MongolRenderEditable(
        ...
        startHandleLayerLink: LayerLink(),
        endHandleLayerLink: LayerLink(),
        offset: rendering.ViewportOffset.fixed(0),
        textSelectionDelegate: delegate,
